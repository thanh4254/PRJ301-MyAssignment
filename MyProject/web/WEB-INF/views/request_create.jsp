...
<%
  ...
  String pFrom   = (String) request.getAttribute("from");
  String pTo     = (String) request.getAttribute("to");
  String pReason = (String) request.getAttribute("reason");
  if (pFrom == null)   pFrom   = "";
  if (pTo == null)     pTo     = "";
  if (pReason == null) pReason = "";

  // escape đơn giản cho textarea
  String escReason = pReason.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;");
%>
<!DOCTYPE html>
<html lang="vi">
<head>
  ...
  <style>
    ...
    .btn-pill{display:inline-flex;align-items:center;gap:8px;padding:10px 16px;border-radius:14px;
    ...
    input[type="date"],textarea{width:100%;box-sizing:border-box;color:#0f172a;background:rgba(255,255,255,.55);
