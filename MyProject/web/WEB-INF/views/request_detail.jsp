...
<%@ page import="model.Request,model.RequestStatus,java.util.Map" %>
<%
...
<head>
  ...
  <style>
    ...
    textarea{width:100%;min-height:160px;color:#0f172a;background:rgba(255,255,255,.55);
  ...
  <div class="glass">
    ...
    <% if (approverName != null && !approverName.isEmpty()) { %>
      <div class="row">
        ...
        <div><strong><%= approverName %></strong><%=
            (approverRole!=null && !approverRole.isEmpty()) ? (", Role: <strong>"+approverRole+"</strong>") : "" %></div>
      </div>
    <% } %>
      ...
      <% } else { %>
        <div class="row muted">
          ...
          <div>
            ...
            <%= "NEW".equals(s) ? "In-progress" : ("APPROVED".equals(s) ? "Approved" : ("REJECTED".equals(s) ? "Rejected" : s)) %>
          </div>
        </div>
      <% } %>
